...
import { useGetAllMatakuliah, useStoreMatakuliah, useUpdateMatakuliah, useDeleteMatakuliah } from "../../../utils/Hooks/useMatakuliah";
import { showInfoToast, showErrorToast } from "../../../utils/toastHelper";
import { showConfirmationDialog } from "../../../utils/swalHelper";
import { useAuthStateContext } from '../../../utils/Contexts/AuthContext';

const ITEMS_PER_PAGE_OPTIONS_MK = [ { value: 5, label: "5" }, { value: 10, label: "10" }, { value: 15, label: "15" } ];
...
const ORDER_OPTIONS_MK = [ { value: "asc", label: "Ascending" }, { value: "desc", label: "Descending" } ];
...
const Matakuliah = () => {
  ...
  const queryParams = useMemo(() => ({ q: debouncedSearchTerm, _sort: sortBy, _order: sortOrder, _page: page, _limit: perPage }), [debouncedSearchTerm, sortBy, sortOrder, page, perPage]);
  const { data: matakuliahResult, isLoading: isLoadingMatakuliahData, isFetching: isFetchingMatakuliah } = useGetAllMatakuliah(queryParams);
  ...
  useEffect(() => {
    const timerId = setTimeout(() => { setDebouncedSearchTerm(searchTerm); if (page !== 1) setPage(1); }, 500);
    return () => clearTimeout(timerId);
  }, [searchTerm, page]);

  const openAddModal = () => { if (!permissions.includes("matakuliah.create")) { showErrorToast("Akses ditolak."); return; } setSelectedMatakuliah(null); setModalOpen(true); };
  const openEditModal = (mk) => { if (!permissions.includes("matakuliah.update")) { showErrorToast("Akses ditolak."); return; } setSelectedMatakuliah(mk); setModalOpen(true); };
  ...
  const handleSubmitFromModal = (formDataFromModal) => {
    ...
    if (!isEditing) {
      if (matakuliahList.some(mk => mk.kodeMk === formDataFromModal.kodeMk)) { showErrorToast(`Kode MK ${formDataFromModal.kodeMk} sudah terdaftar.`); return; }
    }
    ...
    showConfirmationDialog({
      ...
      onConfirm: () => {
        if (isEditing) {
          updateMatakuliahMutate({ id: selectedMatakuliah.id, dataPayload: { id: selectedMatakuliah.id, ...dataToSubmit } }, { onSuccess: () => handleCloseModal() });
        } else {
  ...
  const handleDeleteFromTable = (matakuliahId) => {
    if (!permissions.includes("matakuliah.delete")) { showErrorToast("Akses ditolak."); return; }
    const mkToDelete = matakuliahList.find(mk => mk.id === matakuliahId);
    if (!mkToDelete) return;
    showConfirmationDialog({
      title: 'Konfirmasi Hapus',
      text: `Yakin hapus data mata kuliah ${mkToDelete.namaMk}?`,
  ...
  if (isLoadingAuth) return <Card><p>Memeriksa autentikasi...</p></Card>;
  if (!isAuthenticated || !permissions.includes("matakuliah.page")) return <Card><Heading as="h2">Akses Ditolak</Heading></Card>;

  return (
    <Card>
      <div className="flex justify-between items-center mb-4">
        <Heading as="h2" className="mb-0">Daftar Mata Kuliah</Heading>
      ...
      <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
        <div><Label htmlFor="search-mk">Pencarian</Label><Input id="search-mk" placeholder="Cari..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
        <div><Label htmlFor="sort-by-mk">Urutkan</Label><Select id="sort-by-mk" options={SORT_OPTIONS_MK} value={sortBy} onChange={handleSortByChange} /></div>
        <div><Label htmlFor="sort-order-mk">Urutan</Label><Select id="sort-order-mk" options={ORDER_OPTIONS_MK} value={sortOrder} onChange={handleSortOrderChange} /></div>
        <div><Label htmlFor="per-page-mk">Item/Hal</Label><Select id="per-page-mk" options={ITEMS_PER_PAGE_OPTIONS_MK} value={perPage} onChange={handlePerPageChange} /></div>
      </div>

      {(isLoadingMatakuliahData || isFetchingMatakuliah) && <p>Memuat data mata kuliah...</p>}
      {!isLoadingPage && permissions.includes("matakuliah.read") && (
        <>
          {matakuliahList.length === 0 && !(isLoadingMatakuliahData || isFetchingMatakuliah) && (
            <p className="text-center my-4">{debouncedSearchTerm ? `Tidak ada hasil untuk "${debouncedSearchTerm}".` : "Belum ada data mata kuliah."}</p>
          )}
          {matakuliahList.length > 0 && (
            <MatakuliahTable matakuliahList={matakuliahList} onEdit={openEditModal} onDelete={handleDeleteFromTable} canEdit={permissions.includes("matakuliah.update")} canDelete={permissions.includes("matakuliah.delete")} isLoading={isFetchingMatakuliah} />
          )}
          {totalPages > 0 && matakuliahList.length > 0 && (
            <div className="mt-6 flex justify-between items-center">
              <p className="text-sm">Hal {page} dari {totalPages} (Total: {totalMatakuliah} MK)</p>
              <div className="flex items-center space-x-2">
                <Button onClick={handlePreviousPage} disabled={page === 1 || isActionInProgress} size="sm" variant="secondary">Sebelumnya</Button>
                <Button onClick={handleNextPage} disabled={page === totalPages || isActionInProgress} size="sm" variant="secondary">Selanjutnya</Button>
              </div>
            </div>
          )}
        </>
      )}
      {!isLoadingPage && !permissions.includes("matakuliah.read") && (<p>Anda tidak punya izin.</p>)}

      {(permissions.includes("matakuliah.create") || permissions.includes("matakuliah.update")) && (
        <MatakuliahModal isOpen={isModalOpen} onClose={handleCloseModal} onSubmit={handleSubmitFromModal} selectedMatakuliah={selectedMatakuliah} isSubmitting={isProcessingForm} />
      )}
    </Card>
  );
};
