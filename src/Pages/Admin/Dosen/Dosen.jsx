...
import { useGetAllDosen, useStoreDosen, useUpdateDosen, useDeleteDosen } from "../../../utils/Hooks/useDosen";
import { showInfoToast, showErrorToast } from "../../../utils/toastHelper";
import { showConfirmationDialog } from "../../../utils/swalHelper";
import { useAuthStateContext } from '../../../utils/Contexts/AuthContext';

const ITEMS_PER_PAGE_OPTIONS_DSN = [ { value: 5, label: "5" }, { value: 10, label: "10" }, { value: 15, label: "15" } ];
...
const ORDER_OPTIONS_DSN = [ { value: "asc", label: "Ascending" }, { value: "desc", label: "Descending" } ];
...
const Dosen = () => {
  ...
  const { data: dosenResult, isLoading: isLoadingDosenData, isFetching: isFetchingDosen } = useGetAllDosen(queryParams);
  ...
  const openAddModal = () => { if (!permissions.includes("dosen.create")) { showErrorToast("Akses ditolak."); return; } setSelectedDosen(null); setModalOpen(true); };
  const openEditModal = (dsn) => { if (!permissions.includes("dosen.update")) { showErrorToast("Akses ditolak."); return; } setSelectedDosen(dsn); setModalOpen(true); };
  ...
  const handleSubmitFromModal = (formDataFromModal) => {
    ...
    if (!isEditing) {
      ...
      if (nidnExists) { showErrorToast(`NIDN ${formDataFromModal.nidn} sudah terdaftar.`); return; }
    }
    ...
    showConfirmationDialog({
      ...
      onConfirm: () => {
        if (isEditing) {
          updateDosenMutate({ id: selectedDosen.id, dataPayload: { id: selectedDosen.id, ...dataToSubmit } }, { onSuccess: () => handleCloseModal() });
        } else {
  ...
  const handleDeleteFromTable = (dosenId) => {
    if (!permissions.includes("dosen.delete")) { showErrorToast("Akses ditolak."); return; }
    const dsnToDelete = dosenList.find(d => d.id === dosenId);
    if (!dsnToDelete) return;
    showConfirmationDialog({
      title: 'Konfirmasi Hapus',
      text: `Yakin hapus data dosen ${dsnToDelete.namaDosen}?`,
  ...
  if (!isAuthenticated || !permissions.includes("dosen.page")) return <Card><Heading as="h2">Akses Ditolak</Heading></Card>;

  return (
    <Card>
      <div className="flex justify-between items-center mb-4">
        <Heading as="h2" className="mb-0">Daftar Dosen</Heading>
      ...
      <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
        ...
        <div>
          <Label htmlFor="sort-by-dosen">Urutkan</Label>
        ...
        <div>
          <Label htmlFor="per-page-dosen">Item/Hal</Label>
      ...
      {!isLoadingPage && permissions.includes("dosen.read") && (
        <>
          {dosenList.length === 0 && !(isLoadingDosenData || isFetchingDosen) && (
            <p className="text-center my-4">{debouncedSearchTerm ? `Tidak ada hasil untuk "${debouncedSearchTerm}".` : "Belum ada data dosen."}</p>
          )}
          {dosenList.length > 0 && (
            <DosenTable
              dosenList={dosenList} onEdit={openEditModal} onDelete={handleDeleteFromTable}
              canEdit={permissions.includes("dosen.update")} canDelete={permissions.includes("dosen.delete")}
              isLoading={isFetchingDosen}
            />
          )}
          {totalPages > 0 && dosenList.length > 0 && (
            <div className="mt-6 flex justify-between items-center">
              <p className="text-sm">Hal {page} dari {totalPages} (Total: {totalDosen} dosen)</p>
      ...
      {!isLoadingPage && !permissions.includes("dosen.read") && (<p>Anda tidak punya izin.</p>)}

      {(permissions.includes("dosen.create") || permissions.includes("dosen.update")) && (
        <DosenModal
          isOpen={isModalOpen} onClose={handleCloseModal} onSubmit={handleSubmitFromModal}
          selectedDosen={selectedDosen} isSubmitting={isProcessingForm}
        />
      )}
    </Card>
  );
